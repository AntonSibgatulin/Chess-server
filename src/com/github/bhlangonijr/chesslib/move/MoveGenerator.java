...
public class MoveGenerator {
    ...
    public static Move isValidMove(Board board,String movestr) throws MoveGeneratorException {
        MoveList legalMoves = new MoveList();
        try {
            MoveList moves = generatePseudoLegalMoves(board);
            for (Move move : moves) {
                if (board.isMoveLegal(move, false)) {
                    char[] c = movestr.toCharArray();
                    if(((String.valueOf(c[1])).equals("7")||(String.valueOf(c[1])).equals("2")) && move.toString().length()>4){
                       char[] p = move.toString().toCharArray();
                        System.out.println(move.toString());
                       if((String.valueOf(c[0])).equals(String.valueOf(p[0])) &&
                                (String.valueOf(c[1])).equals(String.valueOf(p[1]))&&
                                (String.valueOf(c[2])).equals(String.valueOf(p[2]))&&
                                (String.valueOf(c[3])).equals(String.valueOf(p[3]))&&
                                (String.valueOf(p[4])).equals("q")|| (String.valueOf(p[4])).equals("Q")){
return move;
                        }
                    }
